...
function renderContent(): HTMLElement {
  ...
  el.innerHTML =
    `<div id="dash-loading" style="color:var(--text-muted);margin:2rem 0">Loading...</div><div id="dash-content" hidden></div>`;
  ...
  Promise.all([getCouncilMembership(ppPublicKey), getTreasury()])
    .then(([membership, treasury]) => {
      ...
      const flags = (config?.jurisdictions || []).map((j) => {
        ...
        }" style="font-size:1.1rem">${flag}</span>`;
      }).join(" ");
      ...
      contentEl.innerHTML = `
        ...
        escapeHtml(membership?.councilName || "Council")
      }</span>
